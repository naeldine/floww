...
def assurer_mois(mois):
    ...
    if resultat is None:
       revenu, objectif = get_config()
        # à toi : récupérer le revenu et l'objectif actuels
        # (indice : tu as déjà une fonction qui fait exactement ça)
        # à toi : insérer une nouvelle ligne dans la table mois
       cur.execute("INSERT INTO mois (mois, revenu, objectif_epargne) VALUES (?, ?, ?)", (mois, revenu, objectif))
        # avec mois, revenu, objectif_epargne — utilise "?" comme d'habitude
       conn.commit()
        # à toi : ne pas oublier de valider la modification
       pass
    conn.close()
...
def total_du_mois(mois):
    ...
    resultat = cur.fetchall()[0][0]  # Récupère le premier élément du tuple
    conn.close()
    return resultat
